...
export const useBlogHooks = () => {
  ...
  // Posts API
  const useFetchPosts = (pageSize = 10, page = 1, filters = {}, search?: string, category?: number) => {
    return useQuery({
      queryKey: ['posts', pageSize, page, filters, search, category],
      queryFn: async () => {
  ...
  const useFetchBlogCategoriesPublic = (pageSize = 100, page = 1, filters = {}) => {
    return useQuery({
      queryKey: ['categories', pageSize, page, filters],
      queryFn: async () => {
        const response = await axiosInstance.get<PaginatedResponse<Category>>('/api/blog/category/', {
