...
export const useLawyerClientsHooks = () => {
  ...
  const getLawyerClients = (
    ...
    return useQuery({
      queryKey: ['lawyer-clients', pageSize, page, search, client_type, ordering, isActive],
      queryFn: () => {
  ...
  const getLawyerClientById = (id: number) => {
    return useQuery({
      ...
      queryFn: async () => {
        const response = await axiosInstance.get(`${API_ROUTES.LAWYER_CLIENTS.getLawyerClients}${id}/`, {
          requiresAuth: true,
          requiredPermission: 'view_lawyerclient'
        } as any)
    ...
    const mutation = useMutation({
      mutationFn: async (lawyerClientData: Partial<LawyerClientTypes & { id: number | undefined }>) => {
        return await axiosInstance.patch(
          `${API_ROUTES.LAWYER_CLIENTS.editLawyerClient}${lawyerClientData?.id}/`,
          lawyerClientData,
          {
            requiresAuth: true,
