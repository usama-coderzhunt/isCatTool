...
export const useDocsHooks = () => {
    ...
    return useQuery({
      ...
      queryFn: () => {
        return axiosInstance.get(API_ROUTES.DOCUMENT.getDocuments, {
          params: { page, page_size: pageSize, ordering, note: search, client_id: clientId },
          requiresAuth: true,
  ...
  const deleteDocType = (id: number | null) => {
    return useMutation({
      mutationFn: () => axiosInstance.delete(API_ROUTES.DOCUMENT_TYPES.deleteDocumentType(id), { requiresAuth: true, requiredPermission: 'delete_documenttype' } as any),
      onSuccess: () => {
