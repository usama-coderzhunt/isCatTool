  ...
  const useUpdateUserPermissions = () => {
    return useMutation({
      mutationFn: async ({ userId, permissions }: { userId: number; permissions: Permission[] }) => {
        return await axiosInstance.put(`/api/users/${userId}/user_permissions/`, { permissions }, {
          requiresAuth: true,
  ...
  const useCreateBusinessUnit = () => {
    const mutation = useMutation({
      mutationFn: ({ name }: { name: string }) => {
        return axiosInstance.post(API_ROUTES.BUSINESS_UNIT.LIST, { name }, {
          requiresAuth: true,
          requiredPermission: 'add_businessunit'
        } as any)
      },
      onSuccess: () => {
  ...
  const useUpdateBusinessUnit = () => {
    const mutation = useMutation({
      mutationFn: ({ id }: { id: number }) => {
        return axiosInstance.put(`${API_ROUTES.BUSINESS_UNIT.LIST}${id}/`, {
          requiresAuth: true,
          requiredPermission: 'change_businessunit'
        } as any)
      },
      onSuccess: () => {
  ...
  const useCreateExtendGroups = () => {
    const mutation = useMutation({
      mutationFn: ({ group, business_unit }: { group: number; business_unit: number }) => {
        return axiosInstance.post(API_ROUTES.BUSINESS_UNIT.EXTENDED_GROUPS, { group, business_unit }, {
          requiresAuth: true,
          requiredPermission: 'add_businessunit'
        } as any)
      },
