...
export const useUserManagementHooks = () => {
  ...
  // User Hooks
  const useUsers = (pageSize: number, page: number, ordering?: string, search?: string) => {
    return useQuery({
  ...
  const deleteGroup = (id: number | null) => {
    return useMutation({
      mutationFn: () => axiosInstance.delete(API_ROUTES.GROUPS.DETAIL(id), { requiresAuth: true, requiredPermission: 'delete_group' } as any),
      onSuccess: () => {
