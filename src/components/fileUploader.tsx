    ...
    <Box {...getRootProps({ className: 'dropzone' })} sx={{ height: 'auto' }}>
      ...
      {localFiles.length ? (
        localFiles.map((file: any) => (
          <div className='w-full p-3 border border-solid border-border rounded-lg flex items-center justify-between bg-backgroundPaper transition-colors duration-200'>
            ...
            {!isEditMode && !file.isExisting && (
              <span
                className='w-6 h-6 bg-transparent flex items-center justify-center cursor-pointer rounded-full hover:bg-primaryLighter transition-colors duration-200'
                onClick={e => {
                  e.stopPropagation()
              ...
              >
                <i className='tabler-x text-[12px] text-primary' />
              </span>
            )}
          </div>
        ))
        ...
        <>
          <div className='flex items-center flex-col p-3 border-2 border-dashed border-border rounded-lg bg-backgroundPaper transition-colors duration-200 cursor-pointer'>
          ...
          {errorMessage && (
            <span className='text-[#ff4c51] text-[12px] leading-[12px] mt-1 font-normal'>{String(errorMessage)}</span>
          )}
        </>
      )}
    </Box>
