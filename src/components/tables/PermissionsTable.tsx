...
const PermissionsTable = ({
  ...
  const tableData = useMemo(() => {
    ...
    // Transform the data
    const transformedData = Object.entries(groupedPermissions).map(
      ([model, permissions]): GroupedPermission => ({
        ...
        isIndeterminate:
          permissions.some(p => checkedPermissions.has(p.id)) && !permissions.every(p => checkedPermissions.has(p.id))
      })
    )
    ...
    <div className='bg-backgroundDefault px-6 py-4'>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
        {row.original.permissions.map(permission => (
          <div key={permission.id} className='flex items-center gap-3 p-2 rounded hover:bg-backgroundPaper'>
            <Checkbox
              className='text-primary'
