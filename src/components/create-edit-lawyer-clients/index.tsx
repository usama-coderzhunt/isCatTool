...
const AddLawyerClientModal: React.FC<AddLawyerClientModalProps> = ({ open, handleClose, title, lawyerClientData, mode, entity }) => {
    // store
    ...
    const onSubmit: SubmitHandler<LawyerClientTypes> = (data: any) => {
        ...
        if (mode === 'create') {
            createLawyerClient(data as LawyerClientTypes, {
                onSuccess: () => {
                    toast.success(`Lawyer ${entity.charAt(0).toUpperCase() + entity.slice(1)} created successfully`)
                    handleCloseModal()
                    setPhoneNumber(null)
                }
            })
            ...
            editLawyerClient(
                ...
                {
                    onSuccess: () => {
                        toast.success(`Lawyer ${entity.charAt(0).toUpperCase() + entity.slice(1)} updated successfully`)
                        handleCloseModal()
                        reset()
                        setPhoneNumber(null)
                    }
                    ...
                    <form noValidate autoComplete='off' className='flex flex-col gap-6' onSubmit={handleSubmit(onSubmit)}>
                        ...
                        {(mode === 'create' || mode === 'edit') && (
                            <div className='w-full flex justify-end'>
                                ...
                                >
                                    {mode === 'create' ? t('lawyerClients.form.createNew', { type: entity }) : t('lawyerClients.form.update', { type: entity })}
                                </Button>
                            </div>
                        )}
                    </form>
