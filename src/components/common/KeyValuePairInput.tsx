...
const KeyValuePairInput: React.FC<KeyValuePairInputProps> = ({
  ...
  const filteredAvailableKeys = availableKeys.filter(key =>
    editingIndex !== null ? key === pairs[editingIndex]?.key || !usedKeys.includes(key) : !usedKeys.includes(key)
  )
      ...
      case 'boolean':
        return (
          <Box className='flex items-center gap-1'>
            <Typography variant='h6' className='text-base font-medium leading-5'>
              {newValue === 'true' ? t('common.yes') : t('common.no')}
            </Typography>
            <Switch
          ...
          {showTypeSelector && (
            <>
              ...
              <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end', width: '100%' }}>
                {editingIndex !== null && (
                  <Button variant='outlined' onClick={handleCancel} disabled={disabled} sx={{ minWidth: 100 }}>
