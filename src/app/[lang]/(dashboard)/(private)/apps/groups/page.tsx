...
const GroupListingPage = () => {
  ...
  const {
    ...
    isLoading,
    error
  } = useGroups(pagination.pageSize, pagination.pageIndex + 1, getOrderingParam(sorting), appliedSearch)

  const handleBulkDelete = (ids: number[]) => {
    if (!ids.length === null) return
    deleteBulkGroups.mutate(ids, {
      onSuccess: () => {
        const newTotalPages = Math.ceil((groupsData?.count - ids.length) / pagination.pageSize)
        ...
        toast.success(t('groups.bulkDeleteSuccessMessage'))
