...
const StaffListings = () => {
  ...
  const { useStaffMembers, useDeleteStaffMember, useStaffPositions, useDeleteStaffMemberBulk, useUpdateStaffStatus } = useStaffHooks()
      ...
      Cell: ({ row }: { row: MRT_Row<Staff> }) => (
        <div className="flex items-center gap-2">
          ...
          {hasPermissions(userPermissions, ['view_staff']) && (
            <IconButton
              onClick={() => router.push(`/${currentLocale}/dashboards/staff/${row.original.id}`)}
              title={t('common.view')}
            >
            ...
            <IconButton
              onClick={() => {
                if (!row.original.user) {
                  toast.error("This staff member has no associated user ID");
                  return;
                }
                router.push(`/${currentLocale}/dashboards/user/${row.original.user}`);
              }}
            >
