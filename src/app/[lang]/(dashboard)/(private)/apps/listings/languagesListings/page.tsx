...
import LanguagesTable from '@/views/pages/languages-comp/languagesTable'
import { useLanguageHooks } from '@/services/useLanguageHooks'

const LanguagesListings = () => {
  ...
  //Api Call
  const { getLanguages } = useLanguageHooks()
  const { data: languagesData, isLoading } = getLanguages(
    pagination.pageSize,
  ...
  return (
    <Grid container spacing={0}>
      <Grid sx={{ width: '100%' }}>
        <LanguagesTable
          languagesData={languagesData?.data?.results ?? []}
          totalRecords={languagesData?.data?.count ?? 0}
...
export default LanguagesListings
