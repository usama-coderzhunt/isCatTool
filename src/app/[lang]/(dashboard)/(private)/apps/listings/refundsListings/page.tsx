...
import RefundsTable from '@/views/apps/commonTable/refundsTable'

const RefundsListings = () => {
  ...
  //hooks
  const { getRefunds } = usePaymentsHooks()
  const { data: refundsData, isLoading } = getRefunds(
    pagination.pageSize,
  ...
  return (
    <Grid container spacing={0}>
      <Grid sx={{ width: '100%' }}>
        <RefundsTable
          refundsData={refundsData?.data?.results ?? []}
          totalRecords={refundsData?.data?.count ?? 0}
...
export default RefundsListings
