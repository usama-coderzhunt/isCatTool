...
import { exportBlogTagsToCSV } from '@/views/apps/commonTable/tableExport'
...
const TagsPage = () => {
  ...
  const handleBulkDelete = async () => {
    await bulkDeleteTags.mutateAsync(selectedTags, {
      onSuccess: () => {
        const newTotalPages = Math.ceil((tagsData?.count || 0) - selectedTags.length / pagination.pageSize)
        ...
        <Card>
          ...
          <Box sx={{ p: 3, pb: 0 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 3, gap: 2, flexWrap: 'wrap' }}>
              ...
              <IconButton onClick={() => exportBlogTagsToCSV(tagsData?.results || [])} title={t('table.export')}>
