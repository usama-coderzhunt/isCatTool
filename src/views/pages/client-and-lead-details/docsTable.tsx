...
const DebouncedInput = ({
    ...
    return <CustomTextField label={t('documents.table.search')} {...props} value={value} onChange={(e) => setValue(e.target.value)} />
}

const DocsTable: React.FC<DocsTableProps> = ({ userPermissions, selectedClientData, rowSelection, setRowSelection, multiple, setMultiple, openDeleteModal, setOpenDeleteModal, setMode, setOpenEditModalOpen, setSelectedDocumentData, setDocumentId }: DocsTableProps) => {
    const { t } = useTranslation('global')
    ...
    return (
        <div className={`w-full ${themeMode === 'light' ? 'customColor' : ''}`}>
            ...
            <DeleteConfModal
                ...
                handleDelete={() =>
                    multiple ? handleBulkDeleteDocuments(Object.keys(rowSelection).map(key => Number(key))) : handleDeleteDocument()
                }
            />
        </div>
    );
