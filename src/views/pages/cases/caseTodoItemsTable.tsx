...
import { Button, Typography, useColorScheme, Tooltip } from '@mui/material'
...
const DebouncedInput = ({
    ...
    return <CustomTextField label={t('common.search')} {...props} value={value} onChange={(e) => setValue(e.target.value)} />
}

const CaseTodoItemsTable: React.FC<CaseTodoItemsTableProps> = ({ handleOpen, setTodoID, todo_Id, filters, filtersApplied, rowSelection, setRowSelection, multiple, setMultiple, deleteModalOpen, setDeleteModalOpen }) => {
    ...
    const { data: todoItemsData, isLoading } = todo_Id
        ? getTodoItems(todo_Id, pagination.pageSize, pagination.pageIndex + 1, getOrderingParam(sorting), globalFilter)
        : getAllTodoItems(pagination.pageSize, pagination.pageIndex + 1, getOrderingParam(sorting), globalFilter, filters, filtersApplied)
    ...
    const handleBulkDeleteTodoItems = (ids: number[]) => {
        ...
        bulkDeleteTodoItems(ids, {
            onSuccess: () => {
                setDeleteModalOpen(false)
                setRowSelection({})
                toast.success('Todo items deleted successfully')
            }
        })
        setMultiple(false)
    }

    const handleActiveStatusChange = (event: React.ChangeEvent<HTMLInputElement>, todoItemId: number, userName: string) => {
    ...
    const handleStatusClick = (todoItemId: number, userName: string, currentStatus: boolean) => {
        handleActiveStatusChange({ target: { checked: !currentStatus } } as React.ChangeEvent<HTMLInputElement>, todoItemId, userName);
    }
    ...
    const columns = [
        {
            ...
            Cell: ({ cell, row }: { cell: MRT_Cell<TodoItem>, row: MRT_Row<TodoItem> }) => {
                ...
                return (
                    <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center cursor-pointer transition-colors ${isCompleted
                            ? 'bg-green-500'
                    ...
                    >
                        {isCompleted && (
                            <i className="tabler-check text-white text-base" />
                        )}
                    </div>
                );
            }
            ...
            Cell: ({ cell }: { cell: MRT_Cell<TodoItem> }) => {
                ...
                return (
                    <Tooltip title={description} arrow>
                        <Typography className='truncate max-w-[200px] w-full'>
                            {description}
                        </Typography>
                    </Tooltip>
                );
    ...
    return (
        <div className={`w-full ${themeMode === 'light' ? 'customColor' : ''}`}>
            ...
            <DeleteConfModal
                ...
                handleDelete={() =>
                    multiple ? handleBulkDeleteTodoItems(Object.keys(rowSelection).map(key => Number(key))) : handleDeleteTodoItem()
                }
            />
            <StatusConfModal
