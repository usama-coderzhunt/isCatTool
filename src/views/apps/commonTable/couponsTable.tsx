...
import { exportCouponsToCSV } from './tableExport'
  ...
  const handleDeleteClickBulk = (ids: number[]) => {
    ...
    bulkDeleteCoupon.mutate(ids, {
      onSuccess: () => {
        ...
        toast.success(t('coupons.table.couponDeletedBulkSuccess'))
        setDeleteModalOpen(false)
        setRowSelection({})
        setMultiple(false)
      }
  ...
  const columns = useMemo<MRT_ColumnDef<CouponsTypes, any>[]>(() => {
    const baseColumns: MRT_ColumnDef<CouponsTypes, any>[] = [
      ...
      },
      {
        ...
        header: t('coupons.table.used_count'),
        Cell: ({ cell }: { cell: MRT_Cell<CouponsTypes> }) => (
      ...
      },
      {
        ...
        header: t('coupons.table.usage_limit'),
        Cell: ({ cell }: { cell: MRT_Cell<CouponsTypes> }) => (
          <Typography variant='body2' className='text-gray-800'>
            {String(cell.getValue() ?? '-')}
          </Typography>
