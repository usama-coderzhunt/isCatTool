...
import { exportGroupsToCSV } from './tableExport'
    ...
    },
    {
      ...
      Cell: ({ row }: { row: MRT_Row<GroupType> }) => (
        <div className='flex items-center gap-2'>
          <IconButton
            onClick={() => router.push(`/${currentLocale}/dashboard/group/${row.original.id}`)}
            title={t('groups.view')}
          >
            <i className='tabler-eye text-xl' />
          </IconButton>
          <IconButton onClick={() => handleDeleteClick(row.original)} disabled={deleteGroupMutation.isPending}>
      ...
      <MaterialReactTable
        ...
        renderTopToolbarCustomActions={() => (
          <div className='flex items-center gap-3'>
            ...
            <IconButton onClick={() => exportGroupsToCSV(groupsData)} title={t('table.export')}>
