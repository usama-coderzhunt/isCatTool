...
const DebouncedInput = ({
  ...
  return <CustomTextField label={t('lawyerClients.table.search')} {...props} value={value} onChange={(e) => setValue(e.target.value)} />
}
...
const LawyerClientTable: FC<LawyerClientTableProps> = ({
  ...
  const handleActiveStatusChange = (event: React.ChangeEvent<HTMLInputElement>, lawyerClientId: number, userName: string) => {
  ...
  const handleStatusConfirm = () => {
    ...
    setData((prevData) =>
      prevData.map((lawyerClient) => (lawyerClient.id === lawyerClientId ? { ...lawyerClient, is_active: newStatus } : lawyerClient))
    )

    updateEditLawyerClient.mutate({ id: lawyerClientId, is_active: newStatus }, {
      onSuccess: () => {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className='flex items-center justify-between'>
        <Typography variant='h3'>
          {pathName?.includes('/lawyer-clients')
            ? t('lawyerClients.title')
            : t('lawyerClients.leads')}
        </Typography>
          ...
          renderTopToolbarCustomActions={() => (
            <div className='flex items-center gap-3'>
              <DebouncedInput
                ...
                placeholder={pathName?.includes('/lawyer-clients') ? t('lawyerClients.table.search') : t('lawyerClients.table.search')}
              />
              <CustomTextField
                select
                id='select-status'
              ...
              <IconButton onClick={() => exportLawyerClientsToCSV(data)}>
      ...
      <AddLawyerClientModal
        ...
        title={
          pathName?.includes('/lawyer-clients')
            ? t('lawyerClients.title') + '  ' + t(`lawyerClients.modes.${mode}`)
            : t('lawyerClients.leads') + '  ' + t(`lawyerClients.modes.${mode}`)
        }
        mode={mode}
