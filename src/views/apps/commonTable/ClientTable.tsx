...
import { MaterialReactTable, MRT_ColumnDef, MRT_Cell, MRT_Row, MRT_SortingState } from 'material-react-table'
...
const CommonTable: FC<CommonTableProps> = ({
  ...
  const handleBulkDelete = (ids: number[]) => {
    ...
    deleteClientMutationBulk.mutate(ids, {
      onSuccess: () => {
        ...
        setRowSelection({})
        toast.success(
          `${pathName?.includes('/clients') ? t('clientTable.clients') : t('clientTable.leads')} ${t('clientTable.deletedSuccess')}`
        )
      }
    })
    setMultiple(false)
  ...
  const handleStatusConfirm = () => {
    ...
    updateClientMutation.mutate(
      { id: clientId, is_active: newStatus },
      {
        onSuccess: () => {
          toast.success(
            `${pathName?.includes('/clients') ? t('clientTable.client') : t('clientTable.lead')} ${t('clientTable.clientStatusUpdated')}`
          )
        }
      }
    )
  ...
  const columns = useMemo<MRT_ColumnDef<Client, any>[]>(
    () => [
      ...
      {
        accessorKey: 'actions',
        Header: () => <span>{t('clientTable.action')}</span>,
        header: 'Action',
        enableSorting: false,
        ...
        Cell: ({ row }: { row: MRT_Row<Client> }) => (
          <div className='flex items-center'>
            ...
            {hasPermissions(userPermissions, ['change_transclient']) && pathName?.includes('lead') && (
            ...
            <div className='flex items-center gap-3'>
              ...
              <IconButton
                onClick={() => {
                  ...
                  exportClientsToCSV(filteredData)
                }}
