...
import { exportCaseTypesToCSV } from './tableExport'
...
const DebouncedInput = ({
  ...
  return (
    <CustomTextField
      label={t('caseTypes.table.search')}
      {...props}
      value={value}
        ...
        Cell: ({ row }: { row: MRT_Row<CaseTypes> }) => (
          <div className='flex items-center gap-1'>
            {hasPermissions(userPermissions, ['delete_casetype']) && (
              <IconButton onClick={() => handleDeleteClick(row.original)} disabled={deleteCaseTypeMutation.isPending}>
  ...
  return (
    <div className='flex flex-col gap-y-8'>
      ...
      <DeleteConfModal
        ...
        title={
          multiple ? t('caseTypes.deleteConfirmationModal.bulkTitle') : t('caseTypes.deleteConfirmationModal.title')
        }
        message={
          multiple ? t('caseTypes.deleteConfirmationModal.bulkMessage') : t('caseTypes.deleteConfirmationModal.message')
        }
