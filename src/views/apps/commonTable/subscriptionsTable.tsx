...
const SubscriptionsTable: FC<SubscriptionsTableProps> = ({
    ...
    const paypalCaptureId = transactionsData?.data?.results[0]?.payment_method === "paypal" ? transactionsData?.data?.results[0]?.payment_details?.paypal_capture_id : null
    ...
    const columns = useMemo<MRT_ColumnDef<SubscriptionsTypes, any>[]>(() => [
        {
            ...
            Cell: ({ cell }: { cell: MRT_Cell<SubscriptionsTypes> }) => (
                <Typography className='font-medium truncate' color='text.primary'>
                    {getDisplayValue(cell.getValue() === "monthly" ? t('subscriptions.table.billing_cycle_monthly') : t('subscriptions.table.billing_cycle_yearly'))}
                </Typography>
            )
        },
        {
            ...
            Cell: ({ row }: { row: MRT_Row<SubscriptionsTypes> }) => {
                ...
                return (
                    <div className='flex items-center gap-2'>
                        {hasPermissions(userPermissions, ['change_subscription']) && canCancel && (
                            <IconButton
                        ...
                        {hasPermissions(userPermissions, ['change_subscription']) &&
                            ['active', 'upgraded'].includes(row.original.status) && (
                                <IconButton
                                    color='primary'
                                    onClick={() => handleRefundClick(row.original.order)}
                                    size='small'
                                >
