...
import { exportDocumentTypesToCSV } from './tableExport'
...
const DebouncedInput = ({
  ...
  return (
    <CustomTextField
      label={t('documents.types.search')}
      {...props}
      value={value}
  ...
  const handleDeleteClickBulk = (ids: number[]) => {
    ...
    deleteDocTypeMutationBulk.mutate(ids, {
      onSuccess: () => {
        ...
        toast.success(t('documents.types.documentTypesBulkDeleted'))
        setDeleteModalOpen(false)
        setRowSelection({})
        setMultiple(false)
      }
        ...
        Cell: ({ row }: { row: MRT_Row<DocumentType> }) => (
          <div className='flex items-center gap-1'>
            {hasPermissions(userPermissions, ['delete_documenttype']) && (
              <IconButton onClick={() => handleDeleteClick(row.original)} disabled={deleteDocTypeMutation.isPending}>
